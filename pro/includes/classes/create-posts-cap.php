...
class URE_Create_Posts_Cap {
    ...
    public function allow_edit_post_type($menu_order) {
        global $_mn_menu_nopriv;
        global $_mn_submenu_nopriv;
        global $pagenow;
        ...
        if (current_user_can($post_type->cap->edit_posts)) {
            unset($_mn_menu_nopriv[$pagenow]);
            unset($_mn_submenu_nopriv[$pagenow][$pagenow]);
        }
    ...
    /**
     * Replace create_posts capability from 'edit_posts' to 'create_posts' for stadnard MN posts,
     * from 'edit_pages' to 'create_pages' for standard MN pages, and do the same for all public custom post types
     * 
     */
    public function activate() {
        ...
        foreach($post_types as $post_type) {
            ...
            $mn_post_types[$post_type->name]->cap->create_posts = self::build_cap_name($post_type);
        }  
