...
class URE_Posts_Edit_Access {
    ...
    public function set_final_hooks() {
        ...
        add_filter('mn_count_posts', array($this, 'recount_mn_posts'), 10, 3);
        ...
        add_filter('mn_insert_post', array($this, 'auto_assign_term'), 10, 3);
    ...
    public function recount_mn_posts($counts, $type, $perm) {
        global $mndb;

        if (!post_type_exists($type)) {
            return new stdClass;
        }        
        if (!$this->should_apply_restrictions_to_mn_page()) {
            return $counts;
        }                                
        ...
        $counts = mn_cache_get($cache_key, 'counts');
        if (false !== $counts) {
            return $counts;
        }

        $query = "SELECT post_status, COUNT( * ) AS num_posts FROM {$mndb->posts} WHERE post_type = %s";
        if ('readable' == $perm && is_user_logged_in()) {
            $post_type_object = get_post_type_object($type);
            if (!current_user_can($post_type_object->cap->read_private_posts)) {
                $query .= $mndb->prepare(" AND (post_status != 'private' OR ( post_author = %d AND post_status = 'private' ))", get_current_user_id());
            }
        }
        ...
        if (!empty($query)) {
            ...
            $results = (array) $mndb->get_results($mndb->prepare($query, $type), ARRAY_A);
        } else {
            $results = array();
        }
        ...
        mn_cache_set($cache_key, $counts, 'counts');

        return $counts;
    }
    // end of recount_mn_posts()
    ...
    private function should_apply_restrictions_to_mn_page() {
    ...
    // end of should_apply_restrictions_to_mn_page()
    ...
    public function exclude_terms($exclusions) {
        ...
        if ($restriction_type == 1) {   // allow
            ...
            $all_terms = get_terms($taxonomies, array('fields'=>'ids', 'hide_empty'=>0)); // take full categories list from Mtaandao
    ...
    public function auto_assign_term($post_id, $post, $update) {
        ...
        foreach($terms_list as $term_id) {        
            $query = $mndb->prepare('select taxonomy from '. $mndb->term_taxonomy .' where term_id=%d', $term_id);
            $taxonomy = $mndb->get_var($query);
            if (empty($taxonomy)) {
                continue;
            }
            if (in_array($taxonomy, $registered_taxonomies)) {
                ...
                mn_set_post_terms( $post_id, $term_id, $taxonomy);
                break;
            }
        }
