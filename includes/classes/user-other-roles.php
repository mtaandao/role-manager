<?php
/*
 * Project: User Role Editor Mtaandao plugin 
...
class URE_User_Other_Roles {
    ...
    public function load_css() {
        ...
        mn_enqueue_style('ure-jquery-multiple-select', plugins_url('/css/multiple-select.css', URE_PLUGIN_FULL_PATH), array(), false, 'screen');
    ...
    public function load_js($hook_suffix)  {
        ...
        mn_register_script('ure-user-profile-other-roles', plugins_url('/js/ure-user-profile-other-roles.js', URE_PLUGIN_FULL_PATH));
        mn_enqueue_script('ure-user-profile-other-roles');
        mn_localize_script('ure-user-profile-other-roles', 'ure_data_user_profile_other_roles', array(
...
<?php 
      ...
      if ($this->lib->user_is_admin($current_user->ID)) {
            echo '<a href="' . mn_nonce_url("users.php?page=users-".URE_PLUGIN_FILE."&object=user&amp;user_id={$user->ID}", "ure_user_{$user->ID}") . '">' . 
