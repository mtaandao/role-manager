...
class URE_Content_View_Restrictions_Posts_List {
    ...
    public function __construct() {
        ...
        add_filter('posts_where', array($this, 'hide_prohibited_posts2'), 10, 2);
        ...
        if (class_exists('EM_Events')) {    // Events Manager plugin ( https://mtaandao.co.ke/plugins/events-manager )
     ...
     * https://mtaandao.co.ke/plugins/events-manager/ 
     */
    ...
    private function get_post_level_data_from_db() {
        ...
        $query = 'select post_id, meta_key, meta_value from '. $mndb->postmeta; 
        if (isset($this->query_vars['post_type']) && $this->query_vars['post_type']!='any' && $this->query_vars['post_type']!='') {
            ...
            $query .= " join {$mndb->posts} on {$mndb->posts}.ID={$mndb->postmeta}.post_id"; 
        } else {
            $post_type = false;
        }
        $query .= " where {$mndb->postmeta}.meta_key='ure_content_for_roles' OR {$mndb->postmeta}.meta_key='ure_prohibit_allow_flag' OR".
                  " {$mndb->postmeta}.meta_key='ure_post_access_error_action'".
                  " order by {$mndb->postmeta}.post_id";
        if (!empty($post_type)) {
            $query .= " AND {$mndb->posts}.post_type='{$post_type}'";
        }
    ...
    private function reverse_posts_list($posts0) {
        ...
        $query = "select ID from {$mndb->posts} where post_status='publish' and ID NOT IN ({$do_not_select})";
        $posts = $mndb->get_col($query);
        if (!is_array($posts)) {
            $posts = array();
        }
    ...
    public function hide_prohibited_posts($mn_query) {
        ...
        if (isset($mn_query->query_vars['post_type'])) {
            $this->post_type = $mn_query->query_vars['post_type'];
        } else {
            $this->post_type = '';
        }
        ...
        if (count($prohibited_posts)>0) {
            $mn_query->set('post__not_in', $prohibited_posts);
        }
    ...
    public function hide_prohibited_posts2($where, $query) {
        ...
        $where .= " AND {$mndb->posts}.ID NOT IN ($post_not_in)";
    ...
    /**
     * Separate restriction for the wlbdash custom post type as it does not use standard Mtaandao query for posts
     */
    public function wlb_dashboard_restrict() {
        
        global $mn_meta_boxes;
        foreach($mn_meta_boxes['dashboard'] as $widgets) {
