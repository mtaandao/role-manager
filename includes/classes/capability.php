...
class URE_Capability {
    ...
    public static function add() {
        ...
        if (!isset($full_capabilities[$cap_id])) {
            ...
            $mn_roles->add_cap($admin_role, $cap_id);
    ...
    public static function delete() {
        ...
        // process users
        $usersId = $mndb->get_col("SELECT $mndb->users.ID FROM $mndb->users");
        ...
        foreach ($mn_roles->role_objects as $mn_role) {
            if ($mn_role->has_cap($capability_id)) {
                $mn_role->remove_cap($capability_id);
            }
        }
