<?php
/*
 * User Role Editor Pro Mtaandao plugin
 * Class URE_Admin_Menu_Copy - creates/stores updated copy of MN admin backend menu for use in admin menu access add-on
...
class URE_Admin_Menu_Copy {
    ...
    private static function get_menu_item_link($menu_key, $submenu_as_parent = true) {
        ...
        if ($submenu_as_parent && !empty($submenu_item) && isset($submenu_item[2])) {
            if (!empty($menu_hook) || 
                (('index.php'!=$submenu_item[2]) && file_exists(PLUGIN_DIR .'/'. $menu_file) && 
            ...
            if (!empty($menu_hook) || 
                (('index.php'!=$menu_key ) && file_exists(PLUGIN_DIR .'/'. $menu_file) && 
                 !file_exists(ABSPATH .'/admin/'. $menu_file))) {                
                self::$admin_is_parent = true;
    ...
    private static function get_submenu_item_link($submenu_key, $menu_key) {
        ...
        if (!empty($menu_hook) || 
            (('index.php'!=$submenu_key) && file_exists(PLUGIN_DIR .'/'. $sub_file) && 
              !file_exists(ABSPATH .'/admin/'. $sub_file))) {
            // If admin.php is the current page or if the parent exists as a file in the plugins or admin dir
            if ((!self::$admin_is_parent && file_exists(PLUGIN_DIR .'/'. $menu_file) && 
                 !is_dir(PLUGIN_DIR .'/'. $menu_key)) || file_exists($menu_file)) {
    ...
    /**
     * Save current Mtaandao admin menu for future use via AJAX requests, when menu is not available
     * 
     */
