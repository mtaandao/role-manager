 ...
 * Class URE_Admin_Menu - support stuff for manipulations with MN admin dashboard menu
...
class URE_Admin_Menu {
    ...
    public static function load_menu_access_data_for_role($role_id) {
    ...
    public static function load_menu_access_data_for_user($user) {
    ...
    public static function save_menu_access_data_for_role($role_id) {
        ...
        foreach (array_keys($menu_access_data) as $role_id) {
            if (!isset($mn_roles->role_names[$role_id])) {
                unset($menu_access_data[$role_id]);
            }
        }
    ...
    public static function get_allowed_caps($allowed_roles, $user) {
        ...
        foreach($allowed_roles as $allowed_role) {
            $allowed_caps = array_merge($allowed_caps, $mn_roles->roles[$allowed_role]['capabilities']);
            if (!empty($user)) {
    ...
    private static function has_cap($cap_required, $allowed_roles, $allowed_caps) {
        // according to class-mn-user.php (#743) - Everyone is allowed to exist.
    ...
    public static function is_separator($type) {
        ...
        $separators = array(
            'mn-menu-separator',    // Mtaandao
            'separator-woocommerce',    // WooCommerce
            'mns_break_menu'    // MN Statistics
        );
