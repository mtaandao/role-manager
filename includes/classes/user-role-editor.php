<?php
/*
 * Main class of User Role Editor Mtaandao plugin
...
class User_Role_Editor {
    ...
    public function plugin_init() {
        ...
        add_action('mn_ajax_ure_ajax', array($this, 'ure_ajax'));
    }
    // end of plugin_init()
...
<?php        
  ...
  public function add_css_to_users_page() {
      ...
      mn_enqueue_style('ure-admin-css', URE_PLUGIN_URL . 'css/ure-admin.css', array(), false, 'screen');
  ...
  public function add_js_to_users_page() {
      ...
      mn_register_script( 'ure-users-js', plugins_url( '/js/ure-users.js', URE_PLUGIN_FULL_PATH ) );
      mn_enqueue_script ( 'ure-users-js' );      
      mn_localize_script( 'ure-users-js', 'ure_users_data', array(
  ...
  public function restore_users_edit_caps($caps, $cap, $user_id, $args) {
    ...
    public function edit_user_permission_check() {
        ...
        if (!is_super_admin($current_user->ID) && is_super_admin($profileuser->ID)) { // trying to edit a superadmin while himself is less than a superadmin
            mn_die(esc_html__('You do not have permission to edit this user.', 'user-role-editor'));
        ...
        if ($pagenow == 'users.php') {
            if ($current_user->has_cap($this->key_capability)) {
                $actions['capabilities'] = '<a href="' .
                        mn_nonce_url("users.php?page=users-" . URE_PLUGIN_FILE . "&object=user&amp;user_id={$user->ID}", "ure_user_{$user->ID}") .
...
// So let's check if user has 'administrator' role better.
    ...
    protected function get_settings_action() {
        ...
        foreach($update_buttons as $update_button) {
            ...
            if (!mn_verify_nonce($_POST['_mnnonce'], 'user-role-editor')) {
    ...
    public function settings() {
        ...
        if (!current_user_can($settings_capability)) {
            mn_die(esc_html__( 'You do not have sufficient permissions to manage options for User Role Editor.', 'user-role-editor' ));
        }
    ...
    public function admin_css_action() {
        ...
        mn_enqueue_style('mn-jquery-ui-dialog');
        mn_enqueue_style('mn-jquery-ui-selectable');
        if (stripos($_SERVER['REQUEST_URI'], 'settings-user-role-editor')!==false) {
            mn_enqueue_style('ure-jquery-ui-tabs', URE_PLUGIN_URL . 'css/jquery-ui-1.10.4.custom.min.css', array(), false, 'screen');
        }
        mn_enqueue_style('ure-admin-css', URE_PLUGIN_URL . 'css/ure-admin.css', array(), false, 'screen');
    }
    ...
    public function edit_roles() {
        ...
        if (!current_user_can('ure_edit_roles')) {
            mn_die(esc_html__('Insufficient permissions to work with User Role Editor', 'user-role-editor'));
        }
    ...
    protected function load_main_page_js() {
        ...
        mn_enqueue_script('jquery-ui-dialog', false, array('jquery-ui-core', 'jquery-ui-button', 'jquery'));
        mn_enqueue_script('jquery-ui-selectable', false, array('jquery-ui-core', 'jquery'));
        mn_register_script('ure-js', plugins_url('/js/ure-js.js', URE_PLUGIN_FULL_PATH));
        mn_enqueue_script('ure-js');
        mn_localize_script('ure-js', 'ure_data', array(
            'mn_nonce' => mn_create_nonce('user-role-editor'),
            'page_url' => $page_url,
            ...
            'numeric_role_name_prohibited' => esc_html__(' Mtaandao does not support numeric Role name (ID). Add latin characters to it.', 'user-role-editor'),
            ...
            'reset_warning' => esc_html__('DANGER! Resetting will restore default settings from Mtaandao Core.', 'user-role-editor') . "\n\n" .
            ...
            'http://role-editor.com/how-to-restore-deleted-mtaandao-user-roles/' . "\n\n" .
