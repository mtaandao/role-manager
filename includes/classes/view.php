...
class URE_View {
    ...
    private function deprecated_show_and_color($cap_id, $builtin_mn_caps, &$label_style, &$hidden_class) {
        
        if (isset($builtin_mn_caps[$cap_id])) {
            if (in_array('deprecated', $builtin_mn_caps[$cap_id])) {
    ...
    public function show_capabilities($for_role = true, $edit_mode=true) {
        ...
        foreach ($full_capabilities as $capability) {    
            ...
            $this->deprecated_show_and_color($cap_id, $built_in_mn_caps, $label_style, $hidden_class);            
            ...
            $cap_groups = $caps_groups_manager->get_cap_groups($cap_id, $built_in_mn_caps);
