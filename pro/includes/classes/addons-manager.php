...
class URE_Addons_Manager {
    ...
    private function init_addons_list() {
        ...
        $this->add('posts_edit');
        $this->add('plugins_activation');   // for user level only
    ...
    private function load_plugins_activation_access_module() {
        if (is_network_admin()) {
            return;
        }
        if (!is_admin()) {
        ...
        if (!empty($activate)) {                                    
            new URE_Plugins_Activation_Access();
            $this->activate('plugins_activation');
        }
    }
