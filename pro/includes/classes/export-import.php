...
<?php
    ...
    public function add_js() {
        mn_register_script( 'ure-js-exp-imp', plugins_url( '/pro/js/ure-js-exp-imp.js', URE_PLUGIN_FULL_PATH ) );
        mn_enqueue_script ( 'ure-js-exp-imp' );
        mn_localize_script( 'ure-js-exp-imp', 'ure_data_exp_imp', 
                array(
...
<?php        
    ...
    protected function save_imported_roles() {
        $apply_to_all = isset($_POST['ure_import_to_all']) ? 1 : 0;
        $this->lib->set_apply_to_all( $apply_to_all );
    ...
    public function import_roles_notification() {
        if ($_REQUEST['action'] == 'roles_import_note') {             
            if ($_REQUEST['result']==1) {
                $this->lib->set_notification( esc_html__('Roles are imported successfully', 'user-role-editor') );
            } elseif (!empty($_REQUEST['message'])) {
                $this->lib->set_notification( sanitize_text_field($_REQUEST['message']) );
            } else {
                $this->lib->set_notification( esc_html__('Unknown error: Roles import was failed', 'user-role-editor') );
            }
        }
    }
