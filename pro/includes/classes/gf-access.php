...
class URE_GF_Access {
    ...
    public function __construct() {
        ...
        $this->user_meta_key = $mndb->prefix . 'ure_allow_gravity_forms';
    ...
    protected function add_gf_import_capability() {
        ...
        $admin_role = $mn_roles->get_role('administrator');
        if (!empty($admin_role) && !$admin_role->has_cap('gravityforms_import')) {        
            $mn_roles->use_db = true;   //  save changes to the database
...
?>
        <script>
            document.location.href = '<?php echo get_option('siteurl') . '/admin/admin.php?page=gf_export'; ?>';
        </script>
<?php                    
                die;
    ...
    public function prohibited_links_redirect() {
        ...
        if ($id>0) {
            ...
            if ( !in_array($id, $allowed_forms_list) ) {    // access to this form is prohibited - redirect user back to the forms list
                // its late to user mn_redirect() ad MN sent some headers already, so use this method for redirection
?>
        <script>
            document.location.href = '<?php echo get_option('siteurl') . '/admin/admin.php?page=gf_edit_forms'; ?>';
        </script>
<?php                    
                die;
            }
