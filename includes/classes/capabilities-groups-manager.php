...
class URE_Capabilities_Groups_Manager {
     ...
     * @return array 
     */
    private function _get_built_in_mn_caps() {
        ...
        if (version_compare($mn_version, '4.4', '<')) {
            $caps['add_users'] = array('core', 'users');  // removed from MN v. 4.4.
        }
        ...
        $caps = apply_filters('ure_built_in_mn_caps', $caps);
        
        $this->built_in_mn_caps = $caps;
        
        return $this->built_in_mn_caps;
    }
    // end of _get_built_in_mn_caps()
    ...
    public function get_cap_groups($cap_id, $built_in_mn_caps=null) {
                
        if (isset($this->built_in_mn_caps[$cap_id])) {
            $groups = $built_in_mn_caps[$cap_id];            
        } else {
