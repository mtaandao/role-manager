...
class URE_Widgets_Show_View {
    ...
    public function override_callback() {
        global $mn_registered_widgets, $mn_registered_widget_controls;

        foreach ($mn_registered_widgets as $id => $widget) {
            if (!isset($mn_registered_widget_controls[$id])) {
                continue;
                //mn_register_widget_control($id,$widget['name'], array($this, 'dummy'));
            }
            $mn_registered_widget_controls[$id]['ure_callback_redirect'] = $mn_registered_widget_controls[$id]['callback'];
            $mn_registered_widget_controls[$id]['callback'] = array($this, 'add_ui_control');
            ...
            array_push($mn_registered_widget_controls[$id]['params'],$id);	
        }
    }
    ...
    public function add_ui_control() {
        ...
        $callback = $mn_registered_widget_controls[$id]['ure_callback_redirect'];
        ...
        if (!empty($params) && isset($params[0]['number'])) {	
            ...
            $id_disp = $mn_registered_widget_controls[$id]['id_base'] .'-'. $number;
        }
        $id_disp .= '_ure_access';
    ...
    public function add_js() {
        ...
        mn_register_script( 'ure-widgets-show-access', plugins_url('/pro/js/widgets-show-access.js', URE_PLUGIN_FULL_PATH ) );
        mn_enqueue_script ( 'ure-widgets-show-access' );
        mn_localize_script( 'ure-widgets-show-access', 'ure_data_widgets_show_access',
                array(
...
?>        
            ...
            <?php mn_nonce_field('user-role-editor', 'ure_nonce'); ?>
        </form>
<?php
