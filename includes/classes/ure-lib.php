<?php
/*
 * Stuff specific for User Role Editor Mtaandao plugin
...
/**
 * This class contains general stuff for usage at Mtaandao plugins
 */
class Ure_Lib extends URE_Base_Lib {
    ...
    public function init_ure_caps() {
        ...
        if (!isset($mn_roles->roles['administrator'])) {
            return;
        }
        ...
        $old_use_db = $mn_roles->use_db;
        $mn_roles->use_db = true;
        $administrator = $mn_roles->role_objects['administrator'];
        ...
        <div class="wrap">
            ...
            <div id="ure_container">                
                <div id="user_role_editor" class="ure-table-cell" >
                    <form id="ure_form" method="post" action="<?php echo URE_ADMIN_URL . URE_PARENT . '?page=users-' . URE_PLUGIN_FILE; ?>" >			
                        <div id="ure_form_controls">
        <?php
        $view->display();
        mn_nonce_field('user-role-editor', 'ure_nonce');
        ?>
...
?>            
    ...
    protected function process_user_request() {
        ...
        if (isset($_POST['action'])) {
            ...
            if ($action == 'reset') {
                ...
                $this->notification = esc_html__('User Roles are restored to Mtaandao default values. ', 'user-role-editor');
    ...
    public function get_default_role() {
        $this->mn_default_role = get_option('default_role');
    }
    ...
    public function get_usermeta_table_name() {
        ...
        $table_name = (!$this->multisite && defined('CUSTOM_USER_META_TABLE')) ? CUSTOM_USER_META_TABLE : $mndb->usermeta;
    ...
    public function has_administrator_role($user_id) {
        ...
        $has_admin_role = $mndb->get_var($query);
        if ($has_admin_role > 0) {
            $result = true;
        } else {
            $result = false;
  ...
  /**
     * return array with Mtaandao user roles
     * 
     * @global MN_Roles $mn_roles
     * @global type $mn_user_roles
     * @return array
     */
...
/*    
    ...
    protected function restore_user_roles() 
    {
        ...
        $query = "select option_value
              ...
              limit 0, 1";
        $option_value = $mndb->get_var($query);
        if ($mndb->last_error) {
            $this->log_event($mndb->last_error, true);
            return $error_message;
        }
        if ($option_value) {
            $query = "update $mndb->options
                    set option_value='$option_value'
                    where option_name='$option_name'
                    limit 1";
            $record = $mndb->query($query);
            if ($mndb->last_error) {
                $this->log_event($mndb->last_error, true);
                return $error_message;
            }
...
*/
    ...
    public function make_roles_backup() 
    {
        ...
        $query = "select option_id
              ...
              where option_name='$backup_option_name'
          limit 0, 1";
        $option_id = $mndb->get_var($query);
        if ($mndb->last_error) {
            $this->log_event($mndb->last_error, true);
            return false;
        }
        if (!$option_id) {
            $roles_option_name = $mndb->prefix.'user_roles';
            $query = "select option_value 
                        from $mndb->options 
                        where option_name like '$roles_option_name' limit 0,1";
            $serialized_roles = $mndb->get_var($query);
            // create user roles record backup            
            $query = "insert into $mndb->options
     ...
     * 
     * @global mndb $mndb   - MN database object
     * @return array 
     */
    ...
    /**
     * return array of built-in MN capabilities (MN 3.1 admin/includes/schema.php) 
     * 
     * @return array 
     */
    public function get_built_in_mn_caps() {
        
        $caps_groups = URE_Capabilities_Groups_Manager::get_instance();                
        $caps = $caps_groups->get_built_in_mn_caps();
        
        return $caps;
    }
    // end of get_built_in_mn_caps()
    ...
    public function get_caps_to_remove() {
        ...
        foreach ($mn_roles->roles as $role) {
        ...
        $caps_to_exclude = $this->get_built_in_mn_caps();
        $ure_caps = $this->get_ure_caps();
        $caps_to_exclude = array_merge($caps_to_exclude, $ure_caps);

        $caps_to_remove = array();
        foreach ($full_caps_list as $capability => $value) {
            if (!isset($caps_to_exclude[$capability])) {    // do not touch built-in MN caps
                // check roles
                $cap_in_use = false;
                foreach ($mn_roles->role_objects as $mn_role) {
                    if ($mn_role->name != 'administrator') {
                        if ($mn_role->has_cap($capability)) {
                            $cap_in_use = true;
                            break;
                        }
                    }
    ...
    protected function capability_help_link($capability) {
        ...
        switch ($capability) {
            case 'activate_plugins':
                $url = 'http://www.shinephp.com/activate_plugins-mtaandao-capability/';
                break;
            case 'add_users':
                $url = 'http://www.shinephp.com/add_users-mtaandao-user-capability/';
                break;
            case 'create_users':
                $url = 'http://www.shinephp.com/create_users-mtaandao-user-capability/';
                break;
            ...
            case 'delete_published_posts':
                $url = 'http://www.shinephp.com/delete-posts-and-pages-mtaandao-user-capabilities-set/';
                break;
            case 'delete_plugins':
                $url = 'http://www.shinephp.com/delete_plugins-mtaandao-user-capability/';
                break;
            case 'delete_themes':
                $url = 'http://www.shinephp.com/delete_themes-mtaandao-user-capability/';
                break;
            case 'delete_users':
                $url = 'http://www.shinephp.com/delete_users-mtaandao-user-capability/';
                break;
            case 'edit_dashboard':
                $url = 'http://www.shinephp.com/edit_dashboard-mtaandao-capability/';
                break;
            case 'edit_files':
                $url = 'http://www.shinephp.com/edit_files-mtaandao-user-capability/';
                break;
            case 'edit_plugins':
                $url = 'http://www.shinephp.com/edit_plugins-mtaandao-user-capability';
                break;
            case 'moderate_comments':
                $url = 'http://www.shinephp.com/moderate_comments-mtaandao-user-capability/';
                break;
            case 'read':
                $url = 'http://shinephp.com/mtaandao-read-capability/';
                break;
            case 'update_core':
                $url = 'http://www.shinephp.com/update_core-capability-for-mtaandao-user/';
                break;
            case 'ure_edit_roles':
    ...
    protected function validate_user_roles() {
        ...
        $users_query = new MN_User_Query(array('fields' => 'ID'));
        ...
        foreach ($users as $user_id) {
            ...
            if (!empty($user_role) && !isset($mn_roles->roles[$user_role])) { // role doesn't exists
    ...
    protected function add_capability_to_full_caps_list($cap_id) {
        if (!isset($this->full_capabilities[$cap_id])) {    // if capability was not added yet
            ...
            if (isset($this->built_in_mn_caps[$cap_id])) {
                $cap['mn_core'] = true;
            } else {
                $cap['mn_core'] = false;
            }
    ...
    /**
     * Add capabilities from user roles save at Mtaandao database
     * 
     */
    ...
    /**
     * Add built-in Mtaandao caps in case some were not included to the roles for some reason
     * 
     */
    protected function add_mtaandao_caps() {
                
        foreach (array_keys($this->built_in_mn_caps) as $cap) {            
            $this->add_capability_to_full_caps_list($cap);
        }        
        
    }
    // end of add_mtaandao_caps()
    
    
    /**
     * Return all available post types except non-public Mtaandao built-in post types
     * 
     * @return array
     */
    ...
    protected function add_custom_post_type_caps() {
        ...
        foreach($post_types as $post_type) {
            ...
            foreach($capabilities as $capability) {
                ...
                if (!$this->multisite &&
                    isset($mn_roles->role_objects['administrator']) && 
                    !isset($mn_roles->role_objects['administrator']->capabilities[$cap_to_check])) {
                    // admin should be capable to edit any posts
                    $mn_roles->role_objects['administrator']->add_cap($cap_to_check, true);
                }                
            }                        
        }
        
        if (!$this->multisite && isset($mn_roles->role_objects['administrator'])) {
            foreach(array('post', 'page') as $post_type_name) {
                ...
                if ($post_type->cap->create_posts!=='edit_'. $post_type->name .'s') {   // 'create' capability is active
                    if (!isset($mn_roles->role_objects['administrator']->capabilities[$post_type->cap->create_posts])) {
                        // admin should be capable to create posts and pages
                        $mn_roles->role_objects['administrator']->add_cap($post_type->cap->create_posts, true);
                    }
                }
            }   // foreach()
        }   // if ()
    ...
    public function init_full_capabilities() {
        ...
        $this->built_in_mn_caps = $this->get_built_in_mn_caps();
        ...
        $this->add_user_caps();
        $this->add_mtaandao_caps();
        $this->add_custom_post_type_caps();
        $this->add_ure_caps();
        
        unset($this->built_in_mn_caps);
    ...
    /**
     * return Mtaandao user roles to its initial state, just like after installation
     * @global MN_Roles $mn_roles
     */
    protected function mn_roles_reinit() {
        global $mn_roles;
        
        $mn_roles->roles = array();
        $mn_roles->role_objects = array();
        $mn_roles->role_names = array();
        $mn_roles->use_db = true;
    ...
    // end of mn_roles_reinit()
    
    /**
     * reset user roles to Mtaandao default roles
     */
    ...
    public function restore_after_blog_switching($blog_id = 0) {
        ...
        $GLOBALS['_mn_switched_stack'] = array();
        $GLOBALS['switched'] = ! empty( $GLOBALS['_mn_switched_stack'] );
    }
    // end of restore_after_blog_switching()
    
    
    protected function mn_api_network_roles_update() {
        global $mndb;
    ...
    /**
     * Update role for all network using Mtaandao API
     * 
     * @return boolean
     */
    ...
    protected function log_event($message, $show_message = false) {
        global $mn_version, $mndb;

        $file_name = URE_PLUGIN_DIR . 'user-role-editor.log';
        $fh = fopen($file_name, 'a');
        $cr = "\n";
        $s = $cr . date("d-m-Y H:i:s") . $cr .
                'Mtaandao version: ' . $mn_version . ', PHP version: ' . phpversion() . ', MySQL version: ' . $mndb->db_version() . $cr;
        fwrite($fh, $s);
    ...
    protected function add_new_role() {
        ...
        if (isset($_POST['user_role_id']) && $_POST['user_role_id']) {
            ...
            if ($numeric_name && ($match[0] == $user_role_id)) { // numeric name discovered
                return esc_html__('Error: Mtaandao does not support numeric Role name (ID). Add latin characters to it.', 'user-role-editor');
            }
            ...
            if ($user_role_id) {
                ...
                if (isset($mn_roles->roles[$user_role_id])) {
                ...
                if (!empty($user_role_copy_from) && $user_role_copy_from != 'none' && $mn_roles->is_role($user_role_copy_from)) {
                    $role = $mn_roles->get_role($user_role_copy_from);
    ...
    protected function rename_role() {
        ...
        $old_role_name = $mn_roles->roles[$user_role_id]['name'];
        $mn_roles->roles[$user_role_id]['name'] = $new_role_name;
        update_option( $mn_roles->role_key, $mn_roles->roles );
    ...
    /**
     * Deletes user role from the MN database
     */
    protected function delete_mn_roles($roles_to_del) {
        global $mn_roles;
        ...
        foreach($roles_to_del as $role_id) {
            ...
            unset($mn_roles->role_objects[$role_id]);
            unset($mn_roles->role_names[$role_id]);
            unset($mn_roles->roles[$role_id]);                
            $result = true;
        }   // foreach()
        if ($result) {
            update_option($mn_roles->role_key, $mn_roles->roles);
    ...
    protected function delete_all_unused_roles() {        
        ...
        $result = $this->delete_mn_roles($roles_to_del);
    ...
    protected function delete_role() {        
        ...
        if (isset($_POST['user_role_id']) && $_POST['user_role_id']) {
            ...
            if ($role==-1) { // delete all unused roles
                ...
                $result = $this->delete_mn_roles(array($role));
            }
            if (empty($result)) {
    ...
    protected function change_default_role() {
        ...
        if (!empty($_POST['user_role_id'])) {
            ...
            if (isset($mn_roles->role_objects[$user_role_id]) && $user_role_id !== 'administrator') {
                ...
                if (empty($result)) {
                    ...
                    $this->get_default_role();
                    $mess = sprintf(esc_html__('Default role for new users is set to %s successfully', 'user-role-editor'), $mn_roles->role_names[$user_role_id]);
                }
...
// Standard Mtaandao capabilities
    ...
    protected function update_user($user) {
        ...
        if (empty($primary_role) || !isset($mn_roles->roles[$primary_role])) {
            $primary_role = '';
        }
        ...
        foreach ($_POST as $key => $value) {
            $result = preg_match('/^mn_role_(.+)/', $key, $match);
            if ($result === 1) {
                $role = $match[1];
                if (isset($mn_roles->roles[$role])) {
                    $user->add_role($role);
    ...
    public function roles_text($roles) {
        ...
        if (is_array($roles) && count($roles) > 0) {
            ...
            foreach ($roles as $role) {
                $role_names[] = $mn_roles->roles[$role]['name'];
            }
...
?>		  
            ...
            <a class="ure_rsb_link" style="background-image:url(<?php echo URE_PLUGIN_URL . 'images/faq-icon.png'; ?>);" target="_blank" href="http://www.shinephp.com/user-role-editor-mtaandao-plugin/#faq"><?php _e('FAQ', 'user-role-editor'); ?></a><br/>
            <hr />
...
<?php         
    ...
    public function user_has_capability($user, $cap) {
        ...
        foreach ($user->roles as $role) {
            ...
            if (!empty($mn_roles->roles[$role]['capabilities'][$cap])) {
                return true;
            }
        }
    ...
    public function show_other_default_roles() {
        ...
        foreach ($this->roles as $role_id => $role) {
            if ( $role_id=='administrator' || $role_id==$this->mn_default_role ) {			
                continue;
            }
    ...
    public function get_ure_page_url() {
        $page_url = URE_ADMIN_URL . URE_PARENT . '?page=users-' . URE_PLUGIN_FILE;
