...
class PluginUpdateChecker_1_5 {
	...
	public $pluginFile = '';  //Plugin filename relative to the plugins directory. Many MN APIs use this to identify plugins.
	...
	public function __construct($metadataUrl, $pluginFile, $slug = '', $checkPeriod = 12, $optionName = '', $muPluginFile = ''){
		...
		$this->debugMode = defined('MN_DEBUG') && MN_DEBUG;
		...
		//it's the same as $pluginFile given that it's not in a subdirectory (MN only looks in the base dir).
	...
	protected function installHooks(){
		...
		//Insert our update info into the update array maintained by MN
		add_filter('site_transient_update_plugins', array($this,'injectUpdate')); //MN 3.0+
		add_filter('transient_update_plugins', array($this,'injectUpdate')); //MN 2.8+

		add_filter('plugin_row_meta', array($this, 'addCheckForUpdatesLink'), 10, 2);
		add_action('admin_init', array($this, 'handleManualCheck'));
		add_action('all_admin_notices', array($this, 'displayManualCheckResult'));

		//Clear the version number cache when something - anything - is upgraded or MN clears the update cache.
		...
		if ( $this->checkPeriod > 0 ){
			...
			if ( !mn_next_scheduled($this->cronHook) && !defined('MN_INSTALLING') ) {
				mn_schedule_event(time(), $scheduleName, $this->cronHook);
			}
			...
			//Like Mtaandao itself, we check more often on certain pages.
			add_action( 'load-update-core.php', array($this, 'maybeCheckForUpdates') );
			
		} else {
			//Periodic checks are disabled.
			mn_clear_scheduled_hook($this->cronHook);
		}
 ...
 */
	...
	/**
	 * Add our custom schedule to the array of Cron schedules used by MN.
	 * 
	...
	/**
	 * Get the name of the update checker's MN-cron hook. Mostly useful for debugging.
	 *
	 * @return string
	 */
	...
	public function requestInfo($queryArgs = array()){
		...
		//Various options for the mn_remote_get() call. Plugins can filter these, too.
		$options = array(
			'timeout' => 10, //seconds
			'headers' => array(
		...
		$pluginInfo = null;
		if ( !is_mn_error($result) && isset($result['response']['code']) && ($result['response']['code'] == 200) && !empty($result['body']) ){
   ...
   $pluginInfo->filename = $this->pluginFile;
			...
			if ( is_mn_error($result) ) {
				$message .= "MN HTTP error: " . $result->get_error_message();
			} else if ( isset($result['response']['code']) ) {
				$message .= "HTTP response code is " . $result['response']['code'] . " (expected: 200)";
			} else {
				$message .= "mn_remote_get() returned an unexpected result.";
			}
	...
	public function maybeCheckForUpdates(){
		...
		if ( current_filter() == 'load-update-core.php' ) {
			...
			//Mtaandao cron schedules are not exact, so lets do a n update check even
	...
	/**
	 * Insert the latest update (if any) into the update list maintained by MN.
	 * 
	...
	public function injectUpdate($updates){
		...
		if ( !empty($update) ) {
			//Let plugins filter the update info before it's passed on to Mtaandao.
			...
			$mnUpdate = $update->toWpFormat();
			$pluginFile = $this->pluginFile;

			if ( $this->isMuPlugin() ) {
				//MN does not support automatic update installation for mu-plugins, but we can still display a notice.
				$mnUpdate->package = null;
				$pluginFile = $this->muPluginFile;
			}
			$updates->response[$pluginFile] = $mnUpdate;
	...
	public function handleManualCheck() {
		...
		if ( $shouldCheck ) {
			...
			mn_redirect(esc_url_raw(add_query_arg(
					array(
	...
	protected function isMuPlugin() {
		...
		if ( $cachedResult === null ) {
			...
			$muPluginDir = realpath(MNMU_PLUGIN_DIR);
	...
	/**
	 * Register a callback for filtering arguments passed to mn_remote_get().
	 * 
	 * The callback function should take one argument - an associative array of arguments -
	 * and return a modified array or arguments. See the MN documentation on mn_remote_get()
	 ...
	 * mn_remote_get (see MN docs for details).
	 *  
...
class PluginInfo_1_3 {
 ...
 public $filename;
	...
	public $id = 0; //The native MN.org API returns numeric plugin IDs, but they're not used for anything.
	...
	/**
	 * Transform plugin info into the format used by the native Mtaandao.org API
	 * 
	 * @return object
	 */
	public function toWpFormat(){
		...
		//as those returned by the native Mtaandao.org API. These can be assigned directly. 
		$sameFormat = array(
...
class PluginUpdate_1_3 {
 ...
 public $tested;
	...
	/**
	 * Transform the update into the format used by Mtaandao native plugin API.
	 * 
	 * @return object
	 */
	public function toWpFormat(){
