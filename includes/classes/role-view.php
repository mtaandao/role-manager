...
class URE_Role_View extends URE_View {
    ...
    public function role_default_prepare_html($select_width=200) {
        ...
        $mn_default_role = $this->lib->get('mn_default_role');
        $this->role_default_html = '<select id="default_user_role" name="default_user_role" '. $select_style .'>';
        foreach ($roles as $key => $value) {
            $selected = $this->lib->option_selected($key, $mn_default_role);
            ...
            if ($show_admin_role || $key != 'administrator') {
                ...
                if ($translated_name === $value['name']) { // get Mtaandao internal translation
...
<?php
                ...
                if ($multisite && !is_network_admin()) {  // Show for single site for MN multisite only
?>
               <hr />
