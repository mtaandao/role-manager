...
class URE_Themes_Access {
    ...
    public function __construct() {
        ...
        $this->user_meta_key = $mndb->prefix . 'ure_allow_themes';
    ...
    public function set_final_hooks() {
        ...
        if ( $this->user_can_activate_themes($current_user) ) {
            ...
            if (count($allowed_themes_list)>0) {
                add_filter('mn_prepare_themes_for_js', array(&$this, 'restrict_themes_list' ));
            }
        }
    ...
    protected function get_allowed_themes_names($allow_themes) {
        ...
        $themes = mn_prepare_themes_for_js();
    ...
    protected function user_profile_themes_select() {
        ...
        $all_themes = mn_prepare_themes_for_js();
   ...
   /**
    ...
    public function admin_load_js($hook_suffix) {
        ...
        if ($hook_suffix === 'user-edit.php') {
            ...
            mn_register_script('ure-jquery-multiple-select', plugins_url('/js/jquery.multiple.select.js', URE_PLUGIN_FULL_PATH));
            mn_enqueue_script('ure-jquery-multiple-select');
            mn_register_script('ure-user-profile-themes', plugins_url('/pro/js/ure-pro-user-profile-themes.js', URE_PLUGIN_FULL_PATH));
            mn_enqueue_script('ure-user-profile-themes');
            mn_localize_script('ure-user-profile-themes', 'ure_pro_data_themes', array(
    ...
    public function admin_css_action() {        
        ...
        mn_enqueue_style('ure-jquery-multiple-select', plugins_url('/css/pro/multiple-select.css', URE_PLUGIN_FULL_PATH), array(), false, 'screen');
    }
    ...
    public function prohibited_links_redirect() {
        ...
        if ( stripos($_SERVER['REQUEST_URI'], 'admin/themes.php?action')===false ) {
            return;
        }    
...
?>
        <script>
            document.location.href = '<?php echo get_option('siteurl') . '/admin/themes.php'; ?>';
        </script>
<?php                    
                die;
            }
