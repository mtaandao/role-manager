...
class URE_Posts_Edit_Access_User {
    ...
    // save posts edit restrictions when user profile page is updated, as Mtaandao itself doesn't know about it
    public function save_user_restrictions($user_id) {
        ...
        $own_data_only = $this->lib->get_request_var('ure_own_data_only', 'post', 'int');
        ...
        // This line added at version 4.23 (2016-01-25), remove it after 2-3 months as unneded.
    ...
    private function get_transient($item_id) {
        ...
        if (!empty($data) && isset($data[$current_user->ID]) && isset($data[$current_user->ID][$item_id])) {
            $value = $data[$current_user->ID][$item_id];
        } else {
            $value = false;
        }
    ...
    private function set_transient($item_id, $item_value) {
        ...
        if (!isset($data[$current_user->ID]) || !is_array($data[$current_user->ID])) {
            $data[$current_user->ID] = array();
        }
        
        $data[$current_user->ID][$item_id] = $item_value;
    ...
    public function get_restriction_type() {        
        ...
        // get from user meta
        $value = $this->user_meta->get_restriction_type($current_user->ID);                                
        if (empty($value)) {
    ...
    public function get_post_categories_list() {
        ...
        $value = $this->user_meta->get_post_categories_list($current_user->ID);
    ...
    public function get_post_authors_list() {
        ...
        $value = $this->user_meta->get_post_authors_list($current_user->ID);
        $value1 = $this->get_restricted_items_from_roles('authors');
        $value = URE_Utils::concat_with_comma($value, $value1);
        
        $own_data_only = $this->user_meta->get_own_data_only($current_user->ID);
        $value2 = null;
        if ($own_data_only) {
    ...
    private function get_posts_list_by_ids() {
        ...
        $value = $this->user_meta->get_posts_list($current_user->ID);
        if (!is_string($value)) {
            $value = '';
        }
    ...
    private function add_related_wc_orders($posts) {
        ...
        $order_items = $mndb->prefix .'woocommerce_order_items';
        $order_itemmeta = $mndb->prefix .'woocommerce_order_itemmeta';
        $orders = array();
                ...
                $query = "SELECT order_id FROM {$order_items} 
                            WHERE order_item_id in (
                                SELECT order_item_id FROM {$order_itemmeta} 
                                    WHERE meta_key='_product_id' and meta_value={$post->ID})";
                $list = $mndb->get_col($query);
    ...
    private function add_child_pages($posts_list) {
        ...
        $mn_query = new MN_Query();        
        $all_pages = $mn_query->query($args);
        // restore filter back
    ...
    private function exclude_posts_which_can_not_edit($posts_list) {
        ...
        for ($i=0; $i<count($posts_list); $i++) {
            ...
            if (!isset($post_types[$post->post_type]) || 
                user_can($current_user->ID, 'edit_post', $posts_list[$i])) {
                $list[] = $posts_list[$i];
            }
        }
    ...
    public function is_restricted() {
        ...
        $data = get_transient(self::CONTENT_EDIT_ACCESS);
        if (!empty($data) && isset($data[$current_user->ID]) && isset($data[$current_user->ID]['restricted'])) {
            return $data[$current_user->ID]['restricted'];
        }        
        if (empty($data)) {
            $data = array();
        }
        ...
        $data[$current_user->ID]['restricted'] = $restricted;
        set_transient(self::CONTENT_EDIT_ACCESS, $data, self::TRANSIENT_EXPIRE);
    ...
    public function get_attachments_list() {
        ...
        $query = "SELECT ID from $mndb->posts WHERE post_type='attachment' AND ". 
                    "(post_parent in ($parents_list)"; 
        if ($restriction_type==1) {   // Allow
            $query .= " OR (post_parent=0 AND post_author=$current_user->ID)";
        }
        $query .= ')';                
        $this->attachments_list = $mndb->get_col($query);            
