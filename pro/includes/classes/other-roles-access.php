...
class URE_Other_Roles_Access {
    ...
    public function __construct($lib) {
        ...
        // exclude blocked roles from Mtaandao dropdown menus
...
<?php
    ...
    public function add_js() {
        mn_register_script( 'ure-other-roles-access', plugins_url( '/pro/js/ure-pro-other-roles-access.js', URE_PLUGIN_FULL_PATH ) );
        mn_enqueue_script ( 'ure-other-roles-access' );
        mn_localize_script( 'ure-other-roles-access', 'ure_data_other_roles_access',
                array(
...
<?php        
    ...
    /**
     * Exclude blocked roles from Mtaandao drop-down menus
     * @param array $roles
     * @return array
     */
    ...
    private function get_users_to_block($blocked) {
        ...
        $multisite = $this->lib->get('multisite');
        $tableName = (!$multisite && defined('CUSTOM_USER_META_TABLE')) ? CUSTOM_USER_META_TABLE : $mndb->usermeta;
    ...
    public function exclude_blocked_roles($user_query) {
        ...
        if (is_array($ids_arr) && count($ids_arr) > 0) {
            $ids = implode(',', $ids_arr);
            $user_query->query_where .= " AND ( {$mndb->users}.ID NOT IN ( $ids ) )";
        }
    }
     ...
     * 1st: http://blogdomain.com/admin/user-edit.php?user_id=ID&mn_http_referer=%2Fadmin%2Fusers.php
     * 2nd: http://blogdomain.com/admin/users.php?action=delete&user=ID&_mnnonce=ab34225a78
